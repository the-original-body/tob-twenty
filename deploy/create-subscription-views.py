...
def create_view(name, object_id, icon, position, existing_views):
    if name in existing_views:
        print(f"  View '{name}' already exists: {existing_views[name]}")
        return existing_views[name]
...
def main():
    ...
    if fields["accessStatus"]:
        ...
        if vid and "Access Not Granted" not in existing:
            add_filter(vid, fields["accessStatus"], "IS", "NOT_GRANTED")

    # --- View 2: Expiring in 60 Days ---
    print("\n--- View 2: Expiring in 60 Days ---")
    if fields["endDate"]:
        vid = create_view("Expiring in 60 Days", sub_id, "IconCalendarDue", 2, existing)
        if vid and "Expiring in 60 Days" not in existing:
            add_filter(vid, fields["endDate"], "IS_RELATIVE", json.dumps({"direction": "NEXT", "amount": 60, "unit": "DAY"}))
    else:
    ...
    if fields["accessStatus"]:
        ...
        if vid and "Paused Subscriptions" not in existing:
            add_filter(vid, fields["accessStatus"], "IS", "PAUSED")

    # --- View 4: Overdue Payments ---
    print("\n--- View 4: Overdue Payments ---")
    if fields["paymentStatus"]:
        vid = create_view("Overdue Payments", sub_id, "IconAlertTriangle", 4, existing)
        if vid and "Overdue Payments" not in existing:
            add_filter(vid, fields["paymentStatus"], "IS", "OVERDUE")
    ...
    if fields["lastTouchpoint"]:
        ...
        if vid and "No Touchpoint 14 Days" not in existing:
            add_filter(vid, fields["lastTouchpoint"], "IS_RELATIVE", json.dumps({"direction": "BEFORE", "amount": 14, "unit": "DAY"}))
